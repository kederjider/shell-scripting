...
# ═══════════════════════════════════════════════════════════
#   🌐  HOST TO IP LOOKUP  -  MENU INTERAKTIF v2.0
# ═══════════════════════════════════════════════════════════
#   🔍 Cari alamat IP yang terhubung dari sebuah domain
# ═══════════════════════════════════════════════════════════
...
# Menu 1: Jalankan Host to IP
run_host_to_ip() {
    print_header "🚀  JALANKAN HOST TO IP"
...
# Menu 2: Lihat hasil (hanya yang dipilih)
view_results() {
    print_header "📊  LIHAT HASIL HOST TO IP"

    local result_dir="hasil_host_to_ip"
    ...
    printf "  ${YELLOW}${BOLD}❓ PILIH${RESET} Nomor file untuk dilihat: "
    read -r choice
    echo
    ...
    if [[ "$choice" =~ ^[0-9]+$ && "$choice" -ge 1 && "$choice" -le ${#file_list[@]} ]]; then
        local selected_file="${file_list[$((choice-1))]}"
        box "$MAGENTA" "📄 Menampilkan hasil: $(basename "$selected_file")"
        echo
        cat "$selected_file"
    else
...
# Menu 3: Hapus hasil yang dipilih
remove_results() {
    print_header "🗑️  HAPUS HASIL YANG DIPILIH"
    ...
    printf "  ${YELLOW}${BOLD}❓ PILIH${RESET} Nomor file untuk dihapus: "
    read -r choice
    echo
